...
import (
	...
	"giovani-milanez/go-expert/auction/internal/entity/bid_entity"
	...
	go func() {
		...
		for {
			...
			case bidEntity, ok := <-bu.bidChannel:
				if !ok {
					logger.Error("bid channel received bid entity with error", nil)
					if len(bidBatch) > 0 {
				...
				logger.Info(fmt.Sprintf("bid entity received: %v, batch size: %d", bidEntity, len(bidBatch)))

				if len(bidBatch) >= bu.maxBatchSize {
					logger.Info(fmt.Sprintf("processing bid batch list, due to max batch size reached: %d", len(bidBatch)))
					if err := bu.BidRepository.CreateBid(ctx, bidBatch); err != nil {
						logger.Error("error trying to process bid batch list", err)
					}
			...
			case <-bu.timer.C:
				logger.Info(fmt.Sprintf("processing bid batch list, due to timer expiration: batch of %d", len(bidBatch)))
				if err := bu.BidRepository.CreateBid(ctx, bidBatch); err != nil {
					logger.Error("error trying to process bid batch list", err)
				}
				bidBatch = nil
